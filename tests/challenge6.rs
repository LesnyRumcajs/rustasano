...
use rustasano::repeating_key_xor::repeating_key_xor_break;
use std::fs;

#[test]
fn should_pass_matasano_6() {
    ...
    let data = fs::read("tests/res/6.txt").expect("Couldn't find test input file");
    let data = base64::decode(&data).expect("Couldn't decode test input file");

    let result = repeating_key_xor_break(&data);
    assert_eq!(expected_plaintext, String::from_utf8(result).unwrap());
}
